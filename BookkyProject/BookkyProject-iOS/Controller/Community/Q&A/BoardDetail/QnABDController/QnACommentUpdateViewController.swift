//
//  QnACommentUpdateViewController.swift
//  BookkyProject-iOS
//
//  Created by 원동진 on 2022/08/16.
//

import UIKit
class QnACommentUpdateViewController: UIViewController,SelectUpdateCommentVCSendData{
...
//        contentsTextView.textColor = UIColor.lightGray
    ...
    @IBAction func tapUpdatePostButton(_ sender: UIButton) {
        ...
        updatePost(textTitle:"title", textContent:contentString , CommunityBoardNumber: self.boardTypeNumber, parentQPID: self.PID, TBID: self.BID, thumbnail: imgarray)
        self.imageArray = []
    ...
    @IBAction func tapUpdateBookButton(_ sender: UIButton) {
        let CommunityStoryBoard = UIStoryboard.init(name: "Community", bundle: nil)
        guard let updateBookViewController = CommunityStoryBoard.instantiateViewController(withIdentifier: "WritePostBookSearchViewController") as? WritePostBookSearchViewController else {return}
        updateBookViewController.updateCommentDelegate = self
        self.present(updateBookViewController, animated: true, completion: nil)
...
extension QnACommentUpdateViewController :UICollectionViewDelegate,UICollectionViewDataSource{
